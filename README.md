...
- **代理组与节点**：查看代理组、切换节点、执行延迟测试，支持按延迟排序、隐藏不可用节点，并保留本地测速历史。
...
- **连接与规则**：查看活动连接、按 TCP/UDP 筛选及时间或流量排序、关闭连接、搜索规则、检查代理与规则提供者。
...
- 检测到非隧道网络接口连续两次不可用时，暂停 ZenClash 管理的内核并释放自身持有的系统代理；网络恢复后启动内核，沿用 TUN 配置并恢复系统代理意图。检测失败视为未知，不根据网站、DNS 或控制器请求失败判定断网。
- 在「内核管理」中手动停止后，不会因联网而自动启动；点击「重启」恢复运行。退出应用会取消自动恢复。
- 后台每秒检查当前配置及启用的 YAML 覆写。有效内容连续两次保持变化后自动校验并重启；注释、格式变化及已由应用应用的内容不会导致重复重启。校验失败保留当前内核，启动失败走现有回滚流程。
- 外部控制器不参与上述自动停止或重启。导入的本地配置沿用托管副本约定，监听对象是当前托管源配置，而非原始导入路径。
- 页面、托盘和历史用量共享最多一秒内的连接快照；关闭连接和内核切换后使快照失效。流量和日志流使用带抖动的递增重连间隔，上限 20 秒。

实现及可重复的手工验证步骤见 [自动运行维护验证](docs/development/automatic-runtime-validation.md)。

## 多控制器、Wi-Fi 规则与菜单栏面板

- 在窗口顶部的「控制器与 Wi-Fi 规则」中添加、编辑、检测和切换远程 Mihomo 控制器。切换先检查版本及运行配置，失败保留原连接；删除当前控制器后返回本机。启动时会尝试恢复上次成功选择的目标。
- 每个远程会话独立持有客户端、连接缓存、流量和日志；切换后释放旧会话。远程页面提供节点、连接、日志与规则，本机内核、系统代理、TUN 和配置仍独立管理。远程流量和日志不写入本机历史。
- macOS 可将精确的 SSID 映射到本地配置。默认关闭；启用后通过系统定位权限读取 Wi-Fi 名称，不采集位置。请使用打包的 `ZenClash.app`；从命令行直接运行时会提示缺少应用权限声明。SSID/链路原生事件触发刷新，另有 30 秒兜底检查；连续两次观察稳定后切换，同一关联只尝试一次。未知 SSID、失效配置、手动停止或远程目标不会触发反复切换。
- 左键点击菜单栏图标打开本机面板，右键保留原生菜单。Linux 托盘不提供鼠标点击事件，可从原生菜单的「打开状态面板」进入。面板展示流量并支持模式、配置、节点、系统代理及 TUN 操作，Esc 或失焦关闭。大节点组可跳转主窗口查看更多。
- `controllers.json` 与 `ssid-rules.json` 独立存储在应用数据目录，旧偏好无需迁移；当前配置备份不包含这两个新增文件。控制器密钥以明文保存，Unix 文件权限为 `0600`，诊断输出遮蔽密钥；请勿分享这两个文件。损坏或未知版本的文件报错并保留原内容。

平台验收步骤及验证边界见 [控制器与菜单栏验收](docs/development/controller-panel-validation.md)。

节点逐项测速分批展示进度，离开节点页取消相关读取和测速；运行时切页取消旧读取，日志筛选结果按数据变化更新；连接列表在后台合并连续输入并筛选排序，更新期间保留上一份列表。展开代理组复用排序索引，测速变化只更新受影响的组。配置表单读取合并后的有效配置；普通回读保留未提交编辑，切换配置或恢复备份时重置编辑状态。普通偏好保存仅同步对应字段；日志、语言、流量历史和网络诊断偏好按各自操作范围管理忙碌状态，内核与配置修改保持串行，备份与这些修改互斥。日志复制与导出在后台格式化，日志导出使用独立忙碌状态，不阻塞内核或配置操作；日志文件大小由后台观察并可能短暂延后显示。响应性优化的行为、自动检查和手工验收步骤见 [UI 响应性验证](docs/development/ui-responsiveness-validation.md)。
...
设置页顶部显示 ZenClash 版本、随包 Mihomo 版本和当前运行内核版本。所有打包脚本自动从安装包版本参数与实际 Mihomo 可执行文件的 `-v` 输出注入版本信息；内核运行版本由 API 获取，停止或离线时显示暂未获取。直接 Cargo 构建使用 workspace 版本，并标记未提供打包信息。
...
- 显式设置 `ZENCLASH_CONTROLLER` 或在控制器界面添加并选择目标后，才会连接外部控制器；已保存的目标会在下次启动时尝试恢复。
